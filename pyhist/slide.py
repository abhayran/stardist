...
class TileGenerator:
    ...
    def execute_and_return(self, **kwargs):
        ...
        if self.method == "graph":
            ...
            raise NotImplementedError
        return self.__return_tiles(mask, bg_color, num_tiles_to_return=None)
    ...
    def __return_tiles(self, mask, bg_color, num_tiles_to_return: Optional[int] = None):
        # Initialize deep zoom generator for the slide
        image_dims = self.input_slide.slide.dimensions
        dzg = deepzoom.DeepZoomGenerator(self.input_slide.slide, tile_size=self.input_slide.patch_size, overlap=0)
        ...
        if num_tiles_to_return is None:
            coord_mem = []
            for row in range(grid_coord[1]):
                for col in range(grid_coord[0]):
                    mask_tile = np.array(dzgmask.get_tile(dzgmask.level_count - 1, (col, row)))
                    if utility_functions.selector(mask_tile, self.input_slide.thres, bg_color, self.input_slide.method):
                        tiles_to_return.append(np.array(dzg.get_tile(dzg_selectedlevel_idx, (col, row))))
                        coord_mem.append([row, col])
            return tiles_to_return, np.array(coord_mem)
        else:
            if not(type(num_tiles_to_return) is int):
                raise ValueError
            coord_mem = set()
